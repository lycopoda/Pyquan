...
class Peak_ID(object):
    def __init__(self, sample, project, code):
	...
	sep = self._project.csv.sep
        line = '{0}{1}'.format(self._ID['code'],sep)
        line += '{0}{1}'.format(self._ID['area'],sep)
        line += '{0}{1}'.format(self._ID['RT'],sep)
        line += '{0}{1}'.format(self._ID['fit'],sep)
        line += '{0}{1}'.format(self._ID['param'],sep)
	try:
	    index = self._ID['real_peak'][0]
	    line += '{0} {1}{2}'.format(index[0],index[1],sep)
	except KeyError:
	    line += '{0}'.format(sep)
	try:
 	    line += '{0}\n'.format(self._ID['real_peak'][1])
	except KeyError:
	    line += ',,\n'
        return line
...
class Code():
    def __init__(self, ID, sample, project):
        self._project = project
	self._sample = sample
        self._ID = ID
	...
	start = self._sample._cdf.time_index(self._x_real[0])
        end = self._sample._cdf.time_index(self._x_real[-1])
	index = [start,end]
	real_x = ''
	real_y = ''
    ...
    def get_data(self):
        import baseline
	time_len = len(self._sample.scan_time) - 2
        t_idx = self._sample.time_index(self._ID.RT)
	int_idx = None
        x = []
        y = []
        i = 0
	for t in range(t_idx-self._window, t_idx+self._window):
            if 0<=t< time_len:
                x.append(self._sample.get_time(t))
                y.append(self._sample.intensity(t,self._ID.mass))
		i+=1
		if t == t_idx:
		    int_idx = i
	if not int_idx:
    ...
    def save_image(self, pars, fit_peak = 'yes'):
        ...
        plot_name = self._project.path.pyquan_fig_file(self._ID.code, self._ID.sample)
        if fit_peak is 'yes':
            for i in pars:
                if pars[i][0]:
