...
#numerical solution to estimate area of EGH, see Lan and Jorgenson, 2001
                t = np.arctan(abs(self._pars['int'][3])/abs(self._pars['int'][2]))
                p=[4., 6.293724, 9.232834, 11.342910, 9.123978, 4.173753, 0.827797]
                epsilon = p[0]-p[1]*t+p[2]*t**2-p[3]*t**3+p[4]*t**4-p[5]*t**5+p[6]*t**6
                area = abs(self._pars['int'][0])*(self._pars['int'][2]*np.sqrt(np.pi/8)+abs(self._pars['int'][3]))*epsilon
                fit = 'yes'
            except:
                area = None
                fit = 'no'
