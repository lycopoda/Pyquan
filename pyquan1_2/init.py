...
#---Calibration-----------------------------------------------------------------
    ...
    def cal_range(self):
        try:
            cal_range = self._init.getfloat('Alignment', 'cal_range')
        except:
            print 'No cal_range defined in pyquan.ini file for automatic RT estimation'
            sys.exit(2)
        return cal_range
...
class Path():
    ...
    def __init__(self, project_name, info):
        ...
        self._info = info
        self._read_csv = info.csv.read_csv 
...
#---files-----------------------------------------------------------------------
	...
	runlist = []
	run_file = self.runlist_file_cal
	try:
	    with open(run_file, 'r') as runfile:
	        for line in runfile:
                    runlist.append(self._read_csv(line)[0].lower())
	except:
	    print 'runlist file not found or corrupt'
	    print 'File name should be project_name.csv, stored in /pyquan/projects'
	    sys.exit(2)
	return runlist
    ...
    @property
    def amdis_file(self):
        return os.path.join(self.root_dir, '{0}_AMDIS.txt'.format(self._project))
	...
	if not file_name:
            print 'amdis file for {0} missing.'.format(sample)
	    sys.exit(2)
        return file_name
        
    def align_data_file(self, item):
        return os.path.join(self.align_dir,'{0}_{1}.csv'.format(self._project,item))
