...
def update_library(amdis_path, lib_path, info, path):
    ...
    with open(lib_path, 'w') as lib_new:
        lib_new.write(info.csv.make_line(['Code','RT','Limit','m/z','Name',
                                         'Source']))
        if info_old:
            for line in info_old:
                try:
                    ...
                    if not all(m in spectrum for m in [int(i) for i in m_list]):
        ...
        if amdis_dict:
            for code in amdis_dict:
                mass = max(mz_dict[code].items(),
                       key=operator.itemgetter(1))[0]
                lib_new.write(info.csv.make_line([amdis.correct_code(code),"","",
